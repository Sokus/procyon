...
static void pe_trace_event_add(const char *name, size_t name_length, peTraceEventType type, uint64_t timestamp, uint64_t duration) {
    ...
    *current_trace_data_used += sizeof(trace_event_data);
    ...
    if (name_length > 0) {
        memcpy(current_trace_data + *current_trace_data_used, name, name_length*sizeof(char));
        *current_trace_data_used += name_length*sizeof(char);
    }
}
