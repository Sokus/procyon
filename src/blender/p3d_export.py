        ...
        for mesh_object in mesh_objects:
            ...
            for polygon in mesh.polygons:
                ...
                for loop_index in polygon.loop_indices:
                    ...
                    if vertex not in p_meshes[mesh_index].vertices:
                        p_meshes[mesh_index].vertices.append(vertex)
    ...
    for mesh in procyon_data.meshes:
        for f in range(int(len(mesh.indices)/3)):
            ...
            for index in mesh.indices[(3*f):(3*f+3)]:
                ...
                if new_vertex not in new_meshes[mesh_index].vertices:
                    new_meshes[mesh_index].vertices.append(new_vertex)
                new_vertex_index = new_meshes[mesh_index].vertices.index(new_vertex)
                new_meshes[mesh_index].indices.append(new_vertex_index)
...
if __name__ == "__main__":
    ...
    p_argument_parser.add_argument("--no-indices", action='store_true', help="don't use indexed vertices")
